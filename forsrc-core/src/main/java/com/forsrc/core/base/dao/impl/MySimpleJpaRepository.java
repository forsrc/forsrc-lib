...
public abstract class MySimpleJpaRepository<T, ID extends Serializable>
        implements JpaRepository<T, ID>, JpaSpecificationExecutor<T> {

    private static final String ID_MUST_NOT_BE_NULL = "The given id must not be null!";

    protected JpaEntityInformation<T, ?> jpaEntityInformation;
    protected EntityManager entityManager;
    protected PersistenceProvider persistenceProvider;
    
    public abstract JpaEntityInformation<T, ?> getJpaEntityInformation();
    public abstract EntityManager getJpaEntityManager();
    ...
    protected Class<T> getDomainClass() {
        return getJpaEntityInformation().getJavaType();
    }

    private String getDeleteAllQueryString() {
        return getQueryString(DELETE_ALL_QUERY_STRING, getJpaEntityInformation().getEntityName());
    }

    private String getCountQueryString() {

        String countQuery = String.format(COUNT_QUERY_STRING, getPersistenceProvider().getCountQueryPlaceholder(), "%s");
        return getQueryString(countQuery, getJpaEntityInformation().getEntityName());
    }
    ...
    public void delete(ID id) {
        ...
        if (entity == null) {
            throw new EmptyResultDataAccessException(
                    String.format("No %s entity with id %s exists!", getJpaEntityInformation().getJavaType(), id), 1);
        }
    ...
    public void delete(T entity) {
        ...
        getJpaEntityManager().remove(getJpaEntityManager().contains(entity) ? entity : getJpaEntityManager().merge(entity));
    }
    ...
    public void deleteInBatch(Iterable<T> entities) {
        ...
        applyAndBind(getQueryString(DELETE_ALL_QUERY_STRING, getJpaEntityInformation().getEntityName()), entities, getJpaEntityManager())
                .executeUpdate();
    }
    ...
    public void deleteAllInBatch() {
        getJpaEntityManager().createQuery(getDeleteAllQueryString()).executeUpdate();
    }
    ...
    public T findOne(ID id) {
        ...
        if (metadata == null) {
            return getJpaEntityManager().find(domainType, id);
        }
        ...
        return type == null ? getJpaEntityManager().find(domainType, id, hints) : getJpaEntityManager().find(domainType, id, type, hints);
    }
    ...
    protected Map<String, Object> getQueryHints() {
        ...
        hints.putAll(Jpa21Utils.tryGetFetchGraphHints(getJpaEntityManager(), getEntityGraph(), getDomainClass()));
    ...
    private JpaEntityGraph getEntityGraph() {
        ...
        String fallbackName = this.getJpaEntityInformation().getEntityName() + "." + metadata.getMethod().getName();
    ...
    public T getOne(ID id) {
        ...
        Assert.notNull(id, ID_MUST_NOT_BE_NULL);
        return getJpaEntityManager().getReference(getDomainClass(), id);
    }
    ...
    public boolean exists(ID id) {
        ...
        if (getJpaEntityInformation().getIdAttribute() == null) {
            return findOne(id) != null;
        }

        String placeholder = getPersistenceProvider().getCountQueryPlaceholder();
        String entityName = getJpaEntityInformation().getEntityName();
        Iterable<String> idAttributeNames = getJpaEntityInformation().getIdAttributeNames();
        String existsQuery = QueryUtils.getExistsQueryString(entityName, placeholder, idAttributeNames);

        TypedQuery<Long> query = getJpaEntityManager().createQuery(existsQuery, Long.class);

        if (!getJpaEntityInformation().hasCompositeId()) {
        ...
        for (String idAttributeName : idAttributeNames) {
            ...
            Object idAttributeValue = getJpaEntityInformation().getCompositeIdAttributeValue(id, idAttributeName);
    ...
    public List<T> findAll(Iterable<ID> ids) {
        ...
        ByIdsSpecification<T> specification = new ByIdsSpecification<T>(getJpaEntityInformation());
     ...
     */
    public long count() {
        return getJpaEntityManager().createQuery(getCountQueryString(), Long.class).getSingleResult();
    }
    ...
    public <S extends T> S save(S entity) {
        ...
        if (getJpaEntityInformation().isNew(entity)) {
            getJpaEntityManager().persist(entity);
            return entity;
        } else {
            return getJpaEntityManager().merge(entity);
        }
    }
    ...
    protected <S extends T> TypedQuery<S> getQuery(Specification<S> spec, Class<S> domainClass, Sort sort) {
        ...
        CriteriaBuilder builder = getJpaEntityManager().getCriteriaBuilder();
        ...
        return applyRepositoryMethodMetadata(getJpaEntityManager().createQuery(query));
    }
    ...
    protected <S extends T> TypedQuery<Long> getCountQuery(Specification<S> spec, Class<S> domainClass) {
        ...
        return getJpaEntityManager().createQuery(query);
    }
